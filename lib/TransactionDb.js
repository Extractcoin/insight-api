...
var genesisTXID = '846e08b063243e3201b25ed09bcada3fb47133182f150d07bd5c3be8303eb72f';
var CONCURRENCY = 10;
...
TransactionDb.prototype.getPoolInfo = function(txid, cb) {
  ...
  Rpc.getTxInfo(txid, function(err, txInfo) {
    ...
    if (txInfo && txInfo.isCoinBase) {
      endPos = txInfo.cbAddresses.length - 7;
      self.strings = {"poolName":'...' + txInfo.cbAddresses.substring(endPos), "url":"#"};
      ret = self.poolMatch.match(new Buffer(txInfo.vin[0].coinbase, 'hex')) ||
        (txInfo.cbAddresses && self.poolMatch.match(new Buffer(txInfo.cbAddresses))) ||
        self.strings;
    }
    return cb(ret);
  });
